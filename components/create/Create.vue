  ...
  <v-container>
    ...
    <v-card flat class="mx-auto mt-10 lightGrey">
      <v-card-title>
        ...
        <SearchResults v-bind:items="searchResultsArr" v-if="showSearchResults" key="search" />
      </v-card-title>
    </v-card>
    <v-card flat class="mx-auto mt-10 lightGrey">
      <v-card-title>
  ...
  methods: {
    async searchRecipes () {
      ...
      if (this.dev) {
        ...
        // this.devRes.forEach(element => {
        //   element.image.replace("https://spoonacular.com", `{spoonacularImageUrl}`);
        //  });
